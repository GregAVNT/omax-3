...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		// icons
					...
					echo '<a href="#TB_inline?width=600&height=450&inlineId=icon-' . esc_attr( $id ) . '"  class="meta-icon-selector"> Select </a>';
					
					echo '<a href="" class="meta-icon-remove" style="display:'.(($meta=='') ? 'none' : '' ).'" > Remove </a>';
					
					echo '</li>';

					echo '<li  id="icon-' . esc_attr( $id ) . '" style="display:none;"><div class="meta-icon-meta-box">';
					foreach ( $options as $option ){
						echo '
						<label for="' . esc_attr( $id ) . '-' . $option . '">
						<input type="radio" id="' . esc_attr( $id ) . '-' . $option . '" value="' . $option . '" ' . checked( $meta, $option, false ) . ' />
						<i class="' . (($meta==$option)?'selected':'')  .' '.$option.'"></i>
						</label>';
					}
					echo '</div></li>';
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		wp_register_style( 'fontawesome', CUSTOM_METABOXES_DIR . '/css/font-awesome.min.css' );
		if ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) )
			$deps[] = 'jqueryui';
		if ( meta_box_find_field_type( 'color', $this->fields ) )
			$deps[] = 'farbtastic';	
		if ( meta_box_find_field_type( 'icons', $this->fields ) )
			$deps[] = 'fontawesome';
	...
	function admin_head() {
		...
		if ( in_array( get_post_type(), $this->page ) && 
			( 
				meta_box_find_field_type( 'date', $this->fields ) 
				|| meta_box_find_field_type( 'slider', $this->fields ) 
				|| meta_box_find_field_type( 'icons', $this->fields ) 
...
break;
					// icons
case 'icons' :
echo '$(".meta_box_items.icons li i").on("click", function(e){
						//e.stopPropagation();
	$(this).closest(\'ul\').find(\'i\').removeClass(\'selected\');
